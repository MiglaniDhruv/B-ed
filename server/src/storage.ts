...
// ─── TTLs: raised significantly to reduce Firestore reads ────────────────────
// Subjects/units/materials change only when an admin edits them (rare).
// Students list changes only on bulk import or status updates (rare).
// Quiz list changes only when admin publishes/edits (rare).
// All of these are invalidated explicitly on mutation anyway, so long TTLs
// are safe — cache is always busted before it naturally expires on writes.
const TTL = {
  SUBJECTS: 10 * 60_000, // 10 min  (was 1 min)
  UNITS: 10 * 60_000, // 10 min  (was 1 min)
  MATERIALS: 10 * 60_000, // 10 min  (was 1 min)
...
export class FirestoreStorage implements IStorage {
  ...
  private docToQuestion(
    ...
    return {
      ...
      order: data.order ?? null,
      createdAt: toDate(data.createdAt) ?? createTime?.toDate() ?? null,
    };
  }
  ...
  async getSemesterStats(semesterNumber: number) {
    ...
    // ── OPTIMIZATION: fetch all units for all subjects in ONE query ────────────
    // Previously: N separate queries (one per subject). Now: 1 query with `in`.
    // Firestore `in` supports up to 30 values — safe for typical subject counts.
    ...
    for (let i = 0; i < subjectIds.length; i += 30) {
      const snap = await this.db
        .collection("units")
        .where("subjectId", "in", subjectIds.slice(i, i + 30))
        .get();
      snap.docs.forEach((d) => allUnits.push(this.docToUnit(d.id, d.data())));
    }

    const unitIds = allUnits.map((u) => u.id);

    // ── OPTIMIZATION: count all materials in ONE query using `in` ──────────────
    // Previously: N count queries (one per unit). Now: 1 query total.
    let materialCount = 0;
    for (let i = 0; i < unitIds.length; i += 30) {
      const snap = await this.db
        ...
        .count()
        .get();
      materialCount += snap.data().count;
    }

    const result = {
      subjectCount: subjects.length,
      chapterCount: semesterNumber === EXAM_PREP_SEMESTER ? 0 : allUnits.length,
      materialCount,
    };
    cache.set(cacheKey, result, TTL.SEMESTER_STATS);
    return result;
  ...
  async getUserAttemptForQuiz(
    ...
    const attempt = this.docToAttempt(snap.docs[0].id, snap.docs[0].data());
  ...
  async getQuizAnalytics(quizId: string): Promise<QuizAnalytics> {
    // ── OPTIMIZATION: cache analytics for 2 min — admin views are infrequent ──
    ...
    // ── OPTIMIZATION: batch-fetch all students in one `in` query ──────────────
    // Previously: one getStudentById() call per attempt = up to 200 reads.
    // Now: batched `in` queries = ceil(uniqueStudents / 30) reads.
    ...
    for (let i = 0; i < studentIds.length; i += 30) {
      const snap = await this.db
        .collection("students")
        .where("__name__", "in", studentIds.slice(i, i + 30))
        .get();
      snap.docs.forEach((d) =>
        studentMap.set(d.id, this.docToStudent(d.id, d.data())),
      );
    }
  ...
  async getNotifications(userId: string): Promise<Notification[]> {
    // ── OPTIMIZATION: cache per-user notification list for 2 min ─────────────
    // Notifications rarely change mid-session. Mutations (markRead, clear)
    // explicitly invalidate this cache key.
    ...
    const result = snap.docs
      .filter((d) => !(d.data().clearedBy ?? []).includes(userId))
      .map((d) => this.docToNotification(d.id, d.data(), userId))
      .sort(
  ...
  async markNotificationRead(id: string, userId: string): Promise<void> {
    ...
    // Invalidate so next fetch reflects updated read state
  ...
  async markAllNotificationsRead(userId: string): Promise<void> {
    ...
    const batch = this.db.batch();
    snap.docs.forEach((d) => {
      batch.update(d.ref, { readBy: FieldValue.arrayUnion(userId) });
    });
    if (!snap.empty) await batch.commit();
    cache.invalidate(`notifications_${userId}`);
  }
  ...
  async notifyAllStudents(
    ...
    await this.db.collection("globalNotifications").add({
      title,
      message,
      type,
      createdAt: new Date(),
    ...
    // Invalidate notification cache for ALL users since this is a new global notification.
    // We invalidate by prefix since we don't know all userIds here.
  ...
  async getUser(id: string) {
    // ── OPTIMIZATION: cache user docs to avoid repeated reads in requireAuth ──
    ...
    if (!doc.exists) return undefined;
    const result = this.docToUser(doc.id, doc.data());
    // Cache for 5 min — invalidated on sessionToken update / password change
  ...
  async deleteUser(id: string) {
    ...
    const batch = this.db.batch();
    [...attempts.docs, ...tokens.docs].forEach((d) => batch.delete(d.ref));
    if (!attempts.empty || !tokens.empty) await batch.commit();
  ...
  // ── Admin Stats ───────────────────────────────────────────────────────────────
  async getAdminStats() {
    // ── OPTIMIZATION: cache admin stats for 2 min ─────────────────────────────
    // Stats page is viewed by admins only; 2 min stale is acceptable.
