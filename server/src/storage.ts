...
export class FirestoreStorage implements IStorage {
  ...
  private docToNotification(
    ...
    return {
      ...
      type: data.type || "info",
      // ✅ isRead: check if userId is in readBy array
  ...
  async getSemesterStats(semesterNumber: number) {
    ...
    if (semesterNumber === EXAM_PREP_SEMESTER) {
  ...
  async getQuizAnalytics(quizId: string): Promise<QuizAnalytics> {
    ...
    const studentList = await Promise.all(
      studentIds.map((sid) => this.getStudentById(sid)),
    );
    const studentMap = new Map(
      studentIds.map((sid, i) => [sid, studentList[i]]),
    );
  ...
  //
  // ✅ NEW: Instead of per-student documents in "notifications" collection,
  // we now use a single "globalNotifications" collection.
  // - 1 document per notification (not 200 documents for 200 students)
  // - readBy[]   → array of userIds who marked it read
  // - clearedBy[] → array of userIds who cleared/dismissed it
  //
  async getNotifications(userId: string): Promise<Notification[]> {
    ...
    const snap = await this.db
      ...
      .limit(50)
      .get();
    console.log("🔍 total docs in globalNotifications:", snap.size);

    return (
      snap.docs
        // ✅ Hide notifications that this user has cleared
  ...
  // ✅ userId parameter added — marks only this user as having read it
  ...
  async clearAllNotifications(userId: string): Promise<void> {
    ...
    const batch = this.db.batch();
    snap.docs.forEach((d) => {
      // ✅ Do NOT delete the document — other students still need it
      // Just mark this user as having cleared + read it
      batch.update(d.ref, {
  ...
  async notifyAllStudents(
    ...
    // ✅ ONE document for ALL students — no more per-student writes
    await this.db.collection("globalNotifications").add({
      ...
      readBy: [], // userIds who read it
      clearedBy: [], // userIds who cleared it
    });
  }
