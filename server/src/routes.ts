...
class RouteCache {
  private store = new Map<string, RouteCacheEntry<any>>();
...
const SEMESTERS_CACHE_TTL = 5 * 60_000;

// ─── Multer (memory only — Firebase pe bhejte hain) ───────────────────────────
const upload = multer({
...
app.post("/api/admin/upload-pdf", upload.single("file"), async (req, res) => {
  ...
  app.get("/api/semesters", requireAuth, async (_req, res) => {
    try {
      ...
      const cached = routeCache.get<any[]>(CACHE_KEY);
      ...
      routeCache.set(CACHE_KEY, semestersWithCounts, SEMESTERS_CACHE_TTL);
  ...
  app.post("/api/admin/units", requireAdminAuth, async (req, res) => {
    try {
      const result = await storage.createUnit(insertUnitSchema.parse(req.body));
      routeCache.invalidate("route_semesters");
      res.json(result);
    } catch (err) {
      if (err instanceof z.ZodError)
  ...
  app.post("/api/admin/study-materials", requireAdminAuth, async (req, res) => {
    try {
      ...
      // data.fileUrl mein Firebase signed URL hogi — MongoDB mein sirf yeh URL save hogi
  ...
  app.get("/api/quizzes/:id/leaderboard", requireAuth, async (req, res) => {
    try {
      const quizId = req.params.id;
      ...
      const myEntry = analytics.leaderboard.find(
        (e) => e.studentId === myUserId,
      );
  ...
  app.put("/api/admin/notices/:id", requireAdminAuth, async (req, res) => {
    try {
      ...
      if ("link" in raw) data.link = raw.link ?? null;
  ...
  app.get("/api/notifications", requireAuth, async (req, res) => {
    try {
      const result = await storage.getNotifications(req.session.userId!);
      res.json(result);
    } catch (err) {
