...
// ─── Helper: local datetime-local string (not UTC) ────────────────────────────
function toLocalISO(d: Date): string {
  const pad = (n: number) => String(n).padStart(2, "0");
  return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`;
}

// ─── Default expiry: current time + 24 hours + 4 minutes ─────────────────────
...
function NoticeForm({
  ...
  const [expiresAt, setExpiresAt] = useState(
    initial?.expiresAt
      ...
      : getDefaultExpiry()   // ✅ current time + 24h + 4min
  );

  // Min allowed = now (can't set expiry in the past)
