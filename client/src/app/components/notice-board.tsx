...
function NoticeForm({
  ...
  const [link, setLink] = useState(initial?.link ?? ""); // ✅ new
  const [expiresAt, setExpiresAt] = useState(
    initial?.expiresAt
      ? toLocalISO(new Date(initial.expiresAt))
      : getDefaultExpiry(),
  );

  const minExpiry = toLocalISO(new Date());
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      priority,
      expiresAt,
      link: link.trim() || null, // ✅ pass link (null if empty)
    });
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex justify-end gap-3 pt-2">
        ...
        <Button
          type="submit"
          disabled={saving}
          className="bg-slate-900 hover:bg-slate-800 text-white px-6 py-2 rounded-lg text-sm flex items-center gap-2"
        >
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1">
            Post notices to all students. They'll receive in-app and push
            notifications.
          </p>
        </div>
        <Button
        ...
        <div className="text-center py-20 border-2 border-dashed border-slate-200 rounded-xl bg-white">
          ...
          <p className="text-sm text-slate-500 mb-4">
            {filter === "expired"
              ...
              : "Create a notice to inform your students."}
          </p>
          {filter !== "expired" && (
            <Button
            ...
            return (
              <div key={notice.id}>
                {editingNotice?.id === notice.id ? (
                  ...
                  <Card
                    className={`p-5 bg-white border-l-4 rounded-xl transition-all ${pCfg.border} ${expired ? "opacity-60" : "hover:shadow-md"}`}
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                          <span
                            className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full ${pCfg.color}`}
                          >
