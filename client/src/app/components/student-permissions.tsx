...
export function StudentPermissions({
  ...
  // parseCsv function - format: name, phone, password (email nahi)
  const parseCsv = (raw: string) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (parts.length < 3) {
        errors.push(`Row ${i + 1}: needs 3 columns (name, phone, password)`);
        continue;
      }
      const [name, phone, password] = parts;
      if (!phone || !password) {
        errors.push(`Row ${i + 1}: phone and password required`);
        continue;
      }
      result.push({ name, email: "", phone, password }); // email blank
    }
...
const handleBulkAdd = async () => {
  ...
  // ── Search: removed enrollmentNumber from filter ──────────────────────────
  ...
  return (
    <div>
      {/* ── Header — single row with all buttons ── */}
      ...
      {modalMode === "bulk" && (
        <Card className="mb-6 p-5 bg-white border-2 border-slate-900 rounded-xl">
          ...
          <p className="text-xs text-slate-400 mb-4">
            Example: John Doe, 9876543210, pass123
          </p>
      ...
      {/* ── Student List — no email, no enrollment ── */}
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Phone only — no email, no enrollment */}
      ...
      {/* ── Edit Modal — no email, no enrollment ── */}
      {editingStudent && (
