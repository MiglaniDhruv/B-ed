...
export function QuizBuilder({
  ...
  const handleShuffleQuestions = async () => {
    if (shuffled || shuffling) return; // one time only
    setShuffling(true);
    try {
      ...
      // Save new order permanently to DB if quiz exists
      if (activeQuizId) {
        const validIds = shuffledQuestions
          .map((q) => q.id)
          .filter((id) => !id.startsWith("q-"));
        if (validIds.length > 0) {
          await api.reorderQuestionsInQuiz(activeQuizId, validIds);
        }
      }
    } catch (err) {
      console.warn("Could not save shuffled order:", err);
      alert(
        "Shuffle applied locally but could not save order to server. It will be saved when you click Save Quiz.",
      );
    } finally {
      setShuffling(false);
    }
  ...
  const handleAddQuestion = async () => {
    ...
    // Reset shuffle state when new question is added
    setShuffled(false);
  ...
  const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Reset shuffle when new questions are imported
      setShuffled(false);
  ...
  const handleDeleteQuestion = async (idx: number) => {
    ...
    // Reset shuffle when a question is deleted
    setShuffled(false);
  ...
  const handleImportQuestions = async (selectedQuestions: Question[]) => {
    ...
    // Reset shuffle when questions are imported from bank
    setShuffled(false);
  ...
  const handleSaveQuiz = async () => {
    ...
    try {
      ...
      if (!isEditMode && !activeQuizId) {
        ...
        // If shuffled, save the final order on save as well
        if (shuffled) {
          ...
          if (validIds.length > 0) {
            try {
              await api.reorderQuestionsInQuiz(quizId, validIds);
            } catch (err) {
              console.warn("Could not save question order:", err);
            }
          }
        }
      }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {questions.length > 0 && (
          <div className="space-y-3">
            {/* ── Questions header with Shuffle button ── */}
            <div className="flex items-center justify-between">
              ...
              {!shuffled ? (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-50 hover:bg-purple-100 disabled:opacity-50 disabled:cursor-not-allowed text-purple-700 border border-purple-200 rounded-lg text-sm font-medium transition-colors"
                >
                  {shuffling ? (
                  ...
                  {shuffling ? "Shuffling..." : "Shuffle Questions"}
                </button>
              ) : (
                <span className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-lg text-sm font-medium">
