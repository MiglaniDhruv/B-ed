...
// ─── Helper: invalidate everything that affects semester card counts ──────────
function invalidateSemesterCounts() {
  cacheDelete("semesters");
...
export function useSubjects(semesterNumber?: number) {
  const { authVersion } = useAuth();
  const cacheKey =
    semesterNumber != null ? `subjects_sem_${semesterNumber}` : "subjects_all";

  const { data, loading, error, refetch } = useCachedFetch(
    cacheKey,
    () =>
      semesterNumber != null
        ? api.getSubjectsBySemester(semesterNumber)
        : api.getSubjects(),
    CACHE_TTL.SUBJECTS,
...
// ─── Notifications ────────────────────────────────────────────────────────────
// OPTIMIZATION: moved onto useCachedFetch with a 2-min TTL.
// Previously this bypassed caching entirely and re-fetched on every
// authVersion bump. Now it serves from localStorage cache and only
// revalidates in the background when stale.
export function useNotifications() {
  const { authVersion, student } = useAuth();

  const client = student ? studentApi : api;
  // Use a role-scoped cache key so admin and student notifications
  // don't overwrite each other in the same browser session.
  const cacheKey = student ? "notifications_student" : "notifications_admin";

  const { data, loading, error, refetch } = useCachedFetch(
    cacheKey,
    () => client.getNotifications(),
    CACHE_TTL.NOTICES, // 5-min TTL (reuse NOTICES TTL)
    [authVersion],
  );
...
// ─── Students (Admin) ─────────────────────────────────────────────────────────
// OPTIMIZATION: moved onto useCachedFetch with a 3-min TTL.
// Previously bypassed caching entirely and re-fetched on every authVersion
// bump. Student list changes infrequently — cache is busted on every mutation.
export function useStudents() {
  ...
  const bust = () => cacheDelete("admin_students");
  ...
  const createStudentsBulk = async (
    bulk: { name: string; email: string; phone: string; password: string }[],
  ) => {
    const result = await api.createStudentsBulk(bulk);
    bust();
    await rawRefetch();
    return result;
  };
