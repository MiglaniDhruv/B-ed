...
class ApiClient {
  ...
  async request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
    ...
    // OPTIMIZATION: Removed the `_t=${Date.now()}` cache-buster that was
    // appended to every GET request. It was defeating browser HTTP caching
    // and making every request look unique to any intermediate cache layer.
    // Session validation is handled server-side via JWT + sessionToken check —
    // we don't need to force-bypass browser cache to stay secure.
  ...
  // Lightweight session validity check — hits /api/auth/ping instead of
  // /api/auth/me. The ping endpoint reads from the server-side in-memory
  // session cache (0 Firestore reads when warm) instead of fetching a full
  // user document. Used by auth-context.ts for the 30-second poll.
  // 🔥 NEW FUNCTION ADD
async uploadPDF(file: File): Promise<{ url: string }> {
  ...
  async ping(): Promise<{ ok: boolean; code?: string }> {
    try {
      return await this.request<{ ok: boolean; code?: string }>(
        "/api/auth/ping",
      );
    } catch (err: any) {
      // Parse code from error message if available
...
class AdminApiClient extends ApiClient {
  ...
  async getStudents(): Promise<Student[]> {
    return this.request("/api/admin/students/with-passwords");
  }
  ...
  async resetStudentPassword(
    ...
    return this.request(`/api/admin/students/${id}/reset-password`, {
      method: "PUT",
  ...
  async updateStudentStatus(
    ...
    return this.request(`/api/admin/students/${id}/status`, {
      method: "PUT",
  ...
  async getSubjectsBySemester(semesterNumber: number): Promise<Subject[]> {
    return this.request(`/api/semesters/${semesterNumber}/subjects`);
  }
  ...
  async updateSubject(id: string, data: Partial<Subject>): Promise<Subject> {
    return this.request(`/api/admin/subjects/${id}`, {
      method: "PUT",
      body: JSON.stringify(data),
    });
  }
  async deleteSubject(id: string): Promise<{ success: boolean }> {
    return this.request(`/api/admin/subjects/${id}`, { method: "DELETE" });
  }
  async getUnitsBySubject(subjectId: string): Promise<Unit[]> {
    return this.request(`/api/subjects/${subjectId}/units`);
  }
  ...
  async updateUnit(id: string, data: Partial<Unit>): Promise<Unit> {
    return this.request(`/api/admin/units/${id}`, {
      method: "PUT",
      body: JSON.stringify(data),
    });
  }
  async deleteUnit(id: string): Promise<{ success: boolean }> {
    return this.request(`/api/admin/units/${id}`, { method: "DELETE" });
  }
  async getStudyMaterialsByUnit(unitId: string): Promise<StudyMaterial[]> {
    return this.request(`/api/units/${unitId}/materials`);
  }
  async createStudyMaterial(data: {
    ...
    return this.request("/api/admin/study-materials", {
      method: "POST",
      body: JSON.stringify(data),
    });
  }
  async updateStudyMaterial(
    ...
    return this.request(`/api/admin/study-materials/${id}`, {
      method: "PUT",
      body: JSON.stringify(data),
    });
  }
  ...
  async createQuiz(data: Omit<Quiz, "id" | "createdAt">): Promise<Quiz> {
  ...
  async deleteQuiz(id: string): Promise<{ success: boolean }> {
    return this.request(`/api/admin/quizzes/${id}`, { method: "DELETE" });
  }
  ...
  async reorderQuestionsInQuiz(
    ...
    return this.request(`/api/admin/quizzes/${quizId}/questions/reorder`, {
      method: "PUT",
  ...
  async clearAllNotifications(): Promise<{ success: boolean }> {
    return this.request("/api/notifications/clear-all", { method: "DELETE" });
  }
  ...
  async updateNotice(
    ...
    return this.request(`/api/admin/notices/${id}`, {
      method: "PUT",
      body: JSON.stringify(data),
    });
  }
  async deleteNotice(id: string): Promise<{ success: boolean }> {
    return this.request(`/api/admin/notices/${id}`, { method: "DELETE" });
  }
...
class StudentApiClient extends ApiClient {
  ...
  async studentLogin(
    ...
    const result = await this.request<{ student: Student; token: string }>(
      "/api/student/login",
  ...
  async checkQuizAttempt(
    quizId: string,
  ): Promise<{ attempted: boolean; attempt: QuizAttempt | null }> {
    return this.request(`/api/quizzes/${quizId}/check-attempt`);
  }
  async submitQuiz(
    quizId: string,
    data: { answers: Record<string, number>; timeTaken: number },
  ): Promise<{ attempt: QuizAttempt; correctAnswers: Record<string, number> }> {
  ...
  async getMyAttempts(): Promise<QuizAttempt[]> {
    return this.request("/api/attempts");
  }
  async getSemesters(): Promise<Semester[]> {
    return this.request("/api/semesters");
  }
